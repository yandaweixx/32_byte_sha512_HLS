...
void do_init(hc_sha512_ctx *ctx0, hc_sha512_ctx *alt_ctx0,
		...
		int salt_len, middle_t *wpc_alt_ctx0, middle_t * wpc_p_bytes0,
		middle_t *wpc_s_bytes0) {
#pragma HLS inline off
...
#pragma HLS LOOP_TRIPCOUNT min=16 max=32
	...
	for (i = 0; i < 8; i++) {
	 wpc_alt_ctx0->buf64[i] = ctx0->state[i];
	 wpc_p_bytes0->buf64[i] = p_bytes0->state[i];
	 wpc_s_bytes0->buf64[i] = s_bytes0->state[i];
	 }
...
void hashing_01800(pwd_t pwd[GROUP_LENGTH], int password0_len[GROUP_LENGTH],int salt_len[GROUP_LENGTH], salt_t salt[GROUP_LENGTH], middle_t wpc_alt_ctx0[GROUP_LENGTH],middle_t wpc_p_bytes0[GROUP_LENGTH],middle_t wpc_s_bytes0[GROUP_LENGTH]) {
...
//#pragma HLS LOOP_FLATTEN off
		tmp_passwd = pwd[i];
		tmp_salt = salt[i];
		tmp_pass_len = tmp_pass_len_1 = password0_len[i];
		tmp_salt_len = tmp_salt_len_1 = salt_len[i];
...
void loop_part(middle_t wpc_alt_ctx0[GROUP_LENGTH],middle_t wpc_p_bytes0[GROUP_LENGTH],middle_t wpc_s_bytes0[GROUP_LENGTH], digest_t digest[GROUP_LENGTH],int passwd_len, int salt_len){

#pragma HLS DATA_PACK variable=wpc_s_bytes0
#pragma HLS DATA_PACK variable=wpc_p_bytes0
#pragma HLS DATA_PACK variable=wpc_alt_ctx0
#pragma HLS DATA_PACK variable=digest

#pragma HLS INTERFACE ap_fifo  port=wpc_s_bytes0
#pragma HLS INTERFACE ap_fifo  port=wpc_p_bytes0
#pragma HLS INTERFACE ap_fifo  port=wpc_alt_ctx0
#pragma HLS INTERFACE ap_fifo  port=digest
	...
	uint8_t inter_alt_ctx0[GROUP_LENGTH][64];
	uint8_t inter_p_bytes0[GROUP_LENGTH][64];
	uint8_t inter_s_bytes0[GROUP_LENGTH][64];

	for(i = 0; i < GROUP_LENGTH; i++)
	{
		tmp_buf1 = wpc_alt_ctx0[i];
		tmp_buf2 = wpc_p_bytes0[i];
		tmp_buf3 = wpc_s_bytes0[i];

		for (m = 0; m < 8; m++) {
			for (j = 7; j >= 0; j--) {
				//#pragma HLS PIPELINE
				inter_alt_ctx0[i][m * 8 + j] = (unsigned char) (((tmp_buf1.buf64[m])
						>> (8 * j)) & 255); //这里并不是真正意义上的uint64_t转unsigned char，没有位变化，只是顺序变为unsigned char
				inter_p_bytes0[i][m * 8 + j] = (unsigned char) (((tmp_buf2.buf64[m])
						>> (8 * j)) & 255);
				inter_s_bytes0[i][m * 8 + j] = (unsigned char) (((tmp_buf3.buf64[m])
						>> (8 * j)) & 255);
			}
		}

		/*for(j = 0; j < 64; j++){
			inter_alt_ctx0[i][j]=tmp_buf1.buf64[j];
			inter_p_bytes0[i][j]=tmp_buf2.buf64[j];
			inter_s_bytes0[i][j]=tmp_buf3.buf64[j];

		}*/
	}
#pragma HLS ARRAY_PARTITION variable=inter_alt_ctx0 complete dim=2
#pragma HLS ARRAY_RESHAPE variable=inter_p_bytes0 complete dim=2
#pragma HLS ARRAY_RESHAPE variable=inter_s_bytes0 complete dim=2


	do_loop(inter_alt_ctx0, inter_p_bytes0, inter_s_bytes0, passwd_len,salt_len);
	digest_t tmp_digest;
#pragma HLS ARRAY_PARTITION variable=tmp_digest.buf64 complete dim=1
//#pragma HLS ARRAY_PARTITION variable=digest->buf64 complete dim=1

	for (i = 0; i < GROUP_LENGTH; i++) {
		for (j = 0; j < 8; j++) {
#pragma HLS PIPELINE II=1
			PUT64BE(tmp_digest.buf64[j], inter_alt_ctx0[i] + j * 8)
